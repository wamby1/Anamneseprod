...
namespace Anamneseprod.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            if( !await _roleManager.RoleExistsAsync(MyRole.Role_User))
            {
                await _roleManager.CreateAsync(new IdentityRole(MyRole.Role_User));
                await _roleManager.CreateAsync(new IdentityRole(MyRole.Role_Admin));
            }
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = CreateUser();
                var myPatientdata = new Patientdata
                {
                    NameFamily = Input.NameFamily,
                ...
                var fhirPatient = ConvertToFhirPatient(myPatientdata);
                var fhirId = await SendToFhirServer(fhirPatient);
                if (!string.IsNullOrEmpty(fhirId))
                {
                    await _userStore.SetUserNameAsync(user, Input.Email, CancellationToken.None);
                    await _emailStore.SetEmailAsync(user, Input.Email, CancellationToken.None);
                    user.NameFamily = Input.NameFamily;
                    user.NameGiven = Input.NameGiven;
